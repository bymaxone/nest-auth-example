...
| 20  | Multi-tenant isolation + workspace switcher      | `tenantIdResolver`, `tenantId` on every entity                                                                    | `X-Tenant-Id` header + Slack-style workspace switcher (`GET /api/account/workspaces` → logout → re-auth at destination tenant) | ✅     |
...
| 40  | Administrative MFA reset                         | `MfaService.resetMfa`                                                                                             | `POST /api/platform/users/:id/reset-mfa` (SUPER_ADMIN)                                                                         | ✅     |
...
| 44  | Authenticated password change                    | `POST /auth/password/change`, `ChangePasswordDto`                                                                 | `dashboard/security` password form (library route; custom `/api/account/change-password` kept as an app-owned reference)       | ✅     |
