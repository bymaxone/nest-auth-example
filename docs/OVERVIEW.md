...
| ----------------- | ------------------------- | -------- | ------------------------------------------------------- |
| Auth library      | `@bymax-one/nest-auth`    | `^1.4.5` | The library this project demonstrates                   |
...
| ORM               | Prisma                    | `^7.7`   | Type-safe, popular in NestJS ecosystem                  |
...
| 7   | Password reset — OTP mode                        | `passwordReset.method: 'otp'`                                                                                     | `?mode=otp` variant of the same pages; the request screen routes into it when `NEXT_PUBLIC_PASSWORD_RESET_MODE=otp`                 | ✅     |
...
| 10  | MFA recovery codes (copy + use)                  | `mfa.recoveryCodeCount`                                                                                           | Modal in `dashboard/security` with a copy control; usable in challenge page                                                         | ✅     |
...
| 14  | Session limit + FIFO eviction                    | `sessions.defaultMaxSessions`                                                                                     | `dashboard/sessions` states the cap (`GET /api/account/session-policy`); e2e test asserts oldest evicted                            | ✅     |
...
| 20  | Multi-tenant isolation + workspace switcher      | `tenantIdResolver`, `tenantId` on every entity                                                                    | `X-Tenant-Id` header + workspace switcher (`GET /api/account/workspaces` → `POST /api/account/switch-workspace` re-issues in place) | ✅     |
...
| 32  | Custom user repository                           | `IUserRepository`                                                                                                 | `PrismaUserRepository` (tenant-scoped object params, lib v1.4.4 port)                                                               | ✅     |
| 33  | Refresh-token reuse detection                    | `onRefreshTokenReuseDetected` hook (family-lineage revoke)                                                        | `AppAuthHooks` → `session.reuse_detected` audit rows                                                                                | ✅     |
| 34  | Login-failure + lockout hooks                    | `onLoginFailed`, `onLockout`                                                                                      | `AppAuthHooks` → `user.login.failed` / `user.lockout` audit rows                                                                    | ✅     |
| 35  | Library-enforced per-IP rate limiting            | `rateLimit.clientIpSource`, `AuthRateLimitGuard`                                                                  | `auth.config.ts` (`peer` source; `AUTH_THROTTLE_DISABLED` opt-out)                                                                  | ✅     |
| 36  | Trusted-origin CSRF guard                        | `cookies.trustedOrigins`, `TrustedOriginGuard`                                                                    | `auth.config.ts` allowlists `WEB_ORIGIN`; e2e asserts `auth.untrusted_origin`                                                       | ✅     |
| 37  | Password policy + breach checking                | `password.minLength/blocklist`, `CommonPasswordChecker`, `HibpBreachChecker`                                      | 15-char floor across API + web forms; `PASSWORD_BREACH_CHECKER=hibp` binds HIBP                                                     | ✅     |
| 38  | Absolute session lifetime                        | `jwt.absoluteSessionLifetimeDays`                                                                                 | `auth.config.ts` (explicit 30-day cap)                                                                                              | ✅     |
| 39  | Account lockout admin surface                    | `AuthService.isAccountLockedOut/getAccountLockoutSeconds/unlockAccount`                                           | Dev `GET/POST/DELETE /api/debug/lockout` endpoints                                                                                  | ✅     |
| 40  | Administrative MFA reset                         | `MfaService.resetMfa`                                                                                             | "Reset 2FA" in `platform/users` → `POST /api/platform/users/:id/reset-mfa` (SUPER_ADMIN)                                            | ✅     |
| 41  | Single-use WebSocket upgrade tickets             | `POST /auth/ws-ticket`, `WsTicketService`                                                                         | `lib/ws-client.ts` mints a ticket per upgrade; gateway redeems it                                                                   | ✅     |
| 42  | WS auth error envelope                           | `WsAuthExceptionFilter`                                                                                           | Class-level filter on `NotificationsGateway`                                                                                        | ✅     |
| 43  | Realtime revocation checks                       | `AuthRevocationService`                                                                                           | Gateway connection-time check (blacklist + token epoch)                                                                             | ✅     |
| 44  | Authenticated password change                    | `POST /auth/password/change`, `ChangePasswordDto`                                                                 | `dashboard/account` password form (library route; custom `/api/account/change-password` kept as an app-owned reference)             | ✅     |
| 45  | Derived HTTP statuses for error codes            | `AUTH_ERROR_STATUS`                                                                                               | `auth-exception.filter` + e2e assertions pin statuses from the map                                                                  | ✅     |
...
| ------ | ---------------------- | ------------------------------------- |
| `main` | `^1.4.5`               | Current stable — consumes npm package |
...
> **Library version targeted:** `@bymax-one/nest-auth@^1.4.5`.
