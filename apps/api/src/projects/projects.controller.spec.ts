 ...
 * verify the metadata so a regression that removes `@Roles` fails here.
 *
 *
 * @layer test
...
describe('ProjectsController', () => {
  ...
  describe('list', () => {
    it('returns the array from listByTenant called with user.tenantId', async () => {
      ...
      // the list is always scoped to the authenticated user's tenant.
  ...
  describe('create', () => {
    it('calls service.create with dto, user.id, and user.tenantId', async () => {
      ...
      // project with the correct owner and tenant.
    ...
    it('has @Roles("ADMIN") metadata so RolesGuard enforces the role gate', () => {
      ...
      // would silently open project creation to all roles.
  ...
  describe('delete', () => {
    it('calls service.delete with the URL param id and user.tenantId', async () => {
      ...
      // do the atomic deleteMany({ id, tenantId }) isolation check.
    ...
    it('propagates NotFoundException from service.delete so NestJS maps it to HTTP 404', async () => {
      ...
      // for a project that was not found, misleading the client.
